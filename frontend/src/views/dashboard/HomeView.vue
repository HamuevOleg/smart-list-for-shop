...
// Shopping Lists with enhanced data
const shoppingLists = computed(() => {
  ...
  return listStore.lists.map((list, index) => {
    ...
    const progress = itemsCount > 0 ? (completed / itemsCount) * 100 : 0;
...
// Mock Chart Data with realistic values
const weeklyActivity = computed(() => {
  const days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
  const now = new Date().getDay();

  return days.map((day, index) => {
    const isPast = index < now;
    const items = isPast ? Math.floor(Math.random() * 20) + 5 : 0;
    return { day, items, isPast };
  });
});

const categoryData = computed(() => [
  { name: 'Fruits', value: 30, color: '#ec4899' },
  { name: 'Dairy', value: 20, color: '#3b82f6' },
  { name: 'Meat', value: 15, color: '#10b981' },
          ...
          <div class="list-header-section">
            ...
            <Transition name="fade">
              <div v-if="showListMenu === list.id" class="list-menu" @click.stop>
                <button @click.stop="deleteList(list.id)" class="menu-item delete">
                  🗑️ Delete
                </button>
              </div>
            </Transition>
...
.modal-leave-active .modal-window {
  animation: modalPop 0.3s cubic-bezier(0.34, 1.56, 0.64, 1) reverse;
}
