  ...
  <div class="sidebar-root">
    ...
    <Teleport to="body">
      <Transition name="fade">
        <div
          v-if="store.isHelpSidebarOpen && isMobile"
...
const startStepAnimation = () => {
  ...
  setTimeout(() => { if (activeStep.value === 2) activeStep.value = 3 }, 4000)

  stepInterval = setInterval(() => {
    activeStep.value = 0
    currentTypedText.value = ''
    setTimeout(() => { activeStep.value = 1; startTypingAnimation() }, 500)
    setTimeout(() => { activeStep.value = 3 }, 4500)
  }, 8000)
}
...
.step.active {
  opacity: 1; transform: scale(1); border-color: rgba(255, 255, 255, 0.15);
  background: rgba(255, 255, 255, 0.06); box-shadow: 0 8px 24px rgba(0, 0, 0, 0.2);
}
.step-header-row { display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem; }
.step-number {
...
.step.active .step-number.ai { background: linear-gradient(135deg, #60a5fa, var(--primary-color)); border-color: var(--primary-color); }
.step-number.success { background: rgba(16, 185, 129, 0.15); border-color: rgba(16, 185, 129, 0.3); }
.step.active .step-number.success { background: #10b981; border-color: #10b981; }
.step-title { font-weight: 700; font-size: 1.05rem; margin: 0; color: #fff; flex: 1; }
.step-body { display: flex; gap: 1rem; align-items: flex-start; }
.step-icon-wrapper { position: relative; width: 52px; height: 52px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.step-icon {
  font-size: 1.8rem; width: 52px; height: 52px; display: flex; align-items: center; justify-content: center;
  border-radius: 14px; background: rgba(255, 255, 255, 0.08); position: relative; z-index: 2; transition: all 0.3s ease;
  border: 1px solid rgba(255, 255, 255, 0.1);
}
.step.active .step-icon { transform: scale(1.05); background: rgba(255, 255, 255, 0.12); border-color: rgba(255, 255, 255, 0.2); }
...
.ai-wrapper { width: 56px; height: 56px; }
.ai-brain { background: linear-gradient(135deg, #2a2a72, var(--primary-color)); animation: brain-pulse 2s ease-in-out infinite; box-shadow: 0 4px 16px rgba(255, 51, 102, 0.3); }
.ai-glow-effect { position: absolute; inset: -12px; background: radial-gradient(circle, var(--primary-color) 0%, transparent 65%); opacity: 0.25; animation: glow-pulse 2s ease-in-out infinite; z-index: 1; border-radius: 50%; filter: blur(8px); }
.brain-waves { position: absolute; inset: 0; z-index: 3; }
.wave { position: absolute; inset: 0; border: 2px solid var(--primary-color); border-radius: 14px; opacity: 0; animation: wave-expand 2s ease-out infinite; }
...
.card-name { font-weight: 700; font-size: 0.95rem; color: #fff; line-height: 1.2; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 110px; }
.card-badges { display: flex; gap: 0.4rem; }
.category-badge { font-size: 0.65rem; background: rgba(16, 185, 129, 0.2); color: #10b981; padding: 2px 6px; border-radius: 6px; font-weight: 600; border: 1px solid rgba(16, 185, 129, 0.3); white-space: nowrap; }
.card-right { display: flex; flex-direction: column; align-items: flex-end; gap: 0.4rem; flex-shrink: 0; }
.card-price { font-size: 0.85rem; font-weight: 700; color: #fff; background: rgba(0, 0, 0, 0.2); padding: 4px 6px; border-radius: 6px; white-space: nowrap; }
.card-check { width: 24px; height: 24px; background: #10b981; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 700; font-size: 0.9rem; animation: check-pop 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) 3.5s backwards; box-shadow: 0 2px 8px rgba(16, 185, 129, 0.4); }

.sparkles { position: absolute; inset: -20px; pointer-events: none; z-index: 3; }
.sparkle { position: absolute; font-size: 0.7rem; animation: sparkle-float 2s ease-in-out infinite; opacity: 0; }
...
.connector { height: 32px; margin-left: 48px; position: relative; }
.connector-line { width: 2px; height: 100%; background: linear-gradient(180deg, rgba(255, 255, 255, 0.08) 0%, rgba(255, 255, 255, 0.04) 100%); margin-left: 15px; }
.data-flow { position: absolute; top: 0; left: 15px; width: 2px; height: 100%; }
.data-particle { position: absolute; top: -6px; left: -3px; width: 8px; height: 8px; background: var(--primary-color); border-radius: 50%; box-shadow: 0 0 12px var(--primary-color); animation: flow-down 2s ease-in-out infinite; }
.data-particle.success { background: #10b981; box-shadow: 0 0 12px #10b981; }

.features-section { padding-top: 1.5rem; border-top: 1px solid rgba(255, 255, 255, 0.08); margin-bottom: 1.25rem; }
.section-header { display: flex; align-items: center; gap: 0.6rem; margin-bottom: 1rem; }
.section-icon { font-size: 1.2rem; }
.section-title { font-size: 1rem; font-weight: 700; margin: 0; color: #fff; }
.feature-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 0.65rem; }
.feature-card { background: rgba(255, 255, 255, 0.04); border: 1px solid rgba(255, 255, 255, 0.08); padding: 0.75rem; border-radius: 12px; display: flex; align-items: center; gap: 0.65rem; transition: all 0.3s ease; cursor: default; }
.feature-card:hover { background: rgba(255, 255, 255, 0.08); border-color: rgba(255, 255, 255, 0.15); transform: translateY(-2px); }
.feature-icon { font-size: 1.3rem; width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; background: rgba(255, 255, 255, 0.08); border-radius: 10px; flex-shrink: 0; }
.feature-text { font-size: 0.85rem; font-weight: 600; color: rgba(255, 255, 255, 0.9); }

.try-it-section { padding-top: 1.25rem; border-top: 1px solid rgba(255, 255, 255, 0.08); }
.btn-try-now { width: 100%; background: linear-gradient(135deg, var(--primary-color), #60a5fa); border: none; color: #fff; padding: 1rem 1.25rem; border-radius: 14px; font-weight: 700; font-size: 0.95rem; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 0.75rem; transition: all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1); box-shadow: 0 6px 24px rgba(255, 51, 102, 0.35); position: relative; overflow: hidden; }
.btn-glow { position: absolute; inset: 0; background: linear-gradient(135deg, rgba(255, 255, 255, 0.2), transparent); opacity: 0; transition: opacity 0.3s ease; }
.btn-try-now:hover .btn-glow { opacity: 1; }
.btn-try-now:hover { transform: translateY(-3px) scale(1.02); box-shadow: 0 12px 32px rgba(255, 51, 102, 0.5); }
.btn-icon { font-size: 1.2rem; }
.btn-text { flex: 1; }

@keyframes blink { 50% { opacity: 0; } }
@keyframes brain-pulse { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.05); } }
@keyframes glow-pulse { 0%, 100% { opacity: 0.25; transform: scale(1); } 50% { opacity: 0.4; transform: scale(1.1); } }
@keyframes wave-expand { 0% { transform: scale(1); opacity: 0.6; } 100% { transform: scale(1.5); opacity: 0; } }
@keyframes success-pop { 0% { transform: scale(0.5); opacity: 0; } 50% { transform: scale(1.2); opacity: 1; } 100% { transform: scale(1); opacity: 1; } }
@keyframes check-pop { 0% { transform: scale(0); } 80% { transform: scale(1.2); } 100% { transform: scale(1); } }
@keyframes sparkle-float { 0%, 100% { transform: translateY(0) scale(0); opacity: 0; } 50% { transform: translateY(-10px) scale(1); opacity: 1; } }
@keyframes flow-down { 0% { top: -6px; opacity: 0; } 20% { opacity: 1; } 80% { opacity: 1; } 100% { top: 100%; opacity: 0; } }
